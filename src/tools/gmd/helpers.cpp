...
std::string bgd::gmd::sanitizeString(std::string const& _text, bool _actually_do_it) {
    if (_actually_do_it) {
        std::string s = _text;
...
std::string bgd::gmd::decodeCompression(std::string const& _text, std::string const& _comp_args) {
...
std::string bgd::gmd::applyCompression(std::string const& _text, std::string const& _comp_args) {
    if (!(_comp_args == "" || _comp_args == "none")) {
        std::vector<uint8_t> decr = convert_vs(_text);
        std::string comps = _comp_args;
...
std::string bgd::gmd::fixPlist(std::string const& _text) {
